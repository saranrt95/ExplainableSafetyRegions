...
def OptimizedSkopeRules(X_train, y_train, param_grid, cvmetric = "accuracy",save_rules=True,rules_path="./best_skope_rules.csv", save_model = True, save_path = "./best_svm.sav"):
    ...
    count = 1
    for rule in rules:
        print("RULE {}: IF ".format(count)+str(rule[0])+" THEN "+class_label+" in 1, COVERING: {}, PRECISION: {}".format(rule[1][1],rule[1][0])+"\n")
        if save_rules:
            with open(rules_path,"a") as rulefile:
                rulefile.write("RULE {}: IF ".format(count)+str(rule[0])+" THEN "+class_label+" in 1, COVERING: {}, PRECISION: {}".format(rule[1][1],rule[1][0])+"\n")
        count+=1
